                ...
                else
                {
                    try
                    {
                        ...
                        if (textBox2.Text == textBox3.Text)
                        {
                            ...
                            IntegrateOS_Data.user = new User(textBox1.Text, encrypt.Get);
                            ...
                            else setResponse = firebaseClient.Set(@"Users/" + username.Get + "_folder/", IntegrateOS_Data.user);
                        }
            ...
            else
            {
                ...
                else
                {
                    try
                    {
                        ...
                        SHA256Encrypt username = new SHA256Encrypt(textBox1.Text);
                        FirebaseResponse firebaseResponse = firebaseClient.Get(@"Users/" + username.Get + "_folder/");
                        IntegrateOS_Data.user = firebaseResponse.ResultAs<User>();
                        if (IntegrateOS_Data.user != null)
                        {
                            if (IntegrateOS_Data.user.Encrypted_Password == encrypt.Get)
                            {
