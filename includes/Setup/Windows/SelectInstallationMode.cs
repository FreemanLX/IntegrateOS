...
{
    public partial class SelectInstallationMode : MetroFramework.Forms.MetroForm
    {
        int j;
        bool folder = false;
        public SelectInstallationMode(System.Drawing.Point punct, int i = 0)
        {
            j = i;
            InitializeComponent();
            Location = punct;
            ...
            if (cancel == false)
            {
                switch (j)
                {
                    ...
                    case 3:
                        ...
                        new LoadingResponse(new Thread(() => Conversion()), "Converting").ShowDialog();
        ...
        private void Search_folder(string hidden_path, int type = 0)
        {
            ...
            new LoadingResponse(new Thread(() =>
            {
                Algorithms.WimFileSearch fileSearch = new Algorithms.WimFileSearch();
                Data.files = fileSearch.SearchingByExtension(hidden_path, extensions);
            }), "Looking for available Windows images").ShowDialog();
            if (Data.files.Count == 0) MessageBox.Show("Not founded available windows images!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            else Moving.Form(this, new Select_Image_File(Data.files, Location, type));
        }
        ...
        private void Back_Click(object sender, EventArgs e)
        {
            ...
            {
                case 0:
                    Moving.Form(this, new PrincipalForm(Location)); break;

                case 3:
                    Moving.Form(this, new SelectInstallationMode(Location, 2)); break;
