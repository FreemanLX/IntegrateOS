...
namespace IntegrateOS.Setup.Windows
{
    public partial class Installation : MetroFramework.Forms.MetroForm
    {
        ...
        private void Installing_Windows_Boot_Files()
        {
            try
            {
                    GenerateProcess.Static_BackgroundWorker_Process("Packages\\bcdboot " + InstallationData.partition + "\\Windows /s " + InstallationData.partition + "\\" + " /f all", "cmd.exe");
                    GenerateProcess.Static_BackgroundWorker_Process("Packages\\bcdedit /copy {current} /d \"IntegrateOS\" > Packages\\bootsector_id.txt", "cmd.exe");
                   ...
                   GenerateProcess.Static_BackgroundWorker_Process("Packages\\bcdedit.exe / set { " + bootsector_id + "} device partition=" + partition + " ", "cmd.exe");
                   GenerateProcess.Static_BackgroundWorker_Process("Packages\\bcdedit.exe / set { " + bootsector_id + "} osdevice partition=" + partition + " ", "cmd.exe");
            }
        ...
        private void Installation_Shown(object sender, EventArgs e)
        {
            ...
            if (check == true)
            {
                GenerateProcess.Static_BackgroundWorker_Process("shutdown -r -t 0", "cmd.exe");
            }
